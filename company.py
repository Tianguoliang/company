...
def get_proxy():
    s=[]
    try:
        abc=requests.get('http://www.xdaili.cn/ipagent//freeip/getFreeIps?page=1&rows=10')
        if abc.status_code==200:
            lines=(abc.json().get('rows'))
            for line in lines:
                ip=line.get('ip')
                port=line.get('port')
                response=str(ip)+':'+str(port)
                s.append(response)
            return s
        return None
...
def search_company_information(keyword,p,count=1):
    print(keyword)
    print(p)
    max_count=10
    global proxy
    ...
    # print (api_headers)
        ...
        for line in s:
            ...
            print(proxy)
            count+=1
            return search_company_information(keyword,p,count)
    try:
...
def main(p):
    ...
    for keyword in keywordlist:
        ...
        if company_informations is not None:
